...
from ..core.exceptions import InvalidInputError, DatabaseConfigurationError
...
class DatabaseConfig:
    ...
    def for_development(cls) -> 'DatabaseConfig':
        """Geliştirme ortamı için konfigürasyon (SQLite in-memory).
        
        Development için hızlı ve izole bir ortam sağlar.
        Her çalıştırmada temiz bir veritabanı oluşturur.
        
        Returns:
            Geliştirme ortamı için DatabaseConfig (SQLite in-memory)
        ...
        """
        dev_engine = EngineConfig.for_development()
        return cls(
            db_name=":memory:",
            ...
            password=None,
            sqlite_path=":memory:",
            engine_config=dev_engine,
        )

    @classmethod
    def for_testing(cls, sqlite_path: str = "test.db") -> 'DatabaseConfig':
        """Test ortamı için konfigürasyon (SQLite file-based).
        
        Test'ler için izole bir veritabanı sağlar.
        Her test çalıştırmasında temiz bir dosya oluşturulabilir.
        
        Args:
            sqlite_path: SQLite dosya yolu (varsayılan: "test.db")
        
        Returns:
            Test ortamı için DatabaseConfig (SQLite file-based)
    ...
    def for_production(
        ...
        Production için güvenli ve optimize edilmiş ayarlar.
        Yüksek eşzamanlılık için optimize edilmiş pool ayarları kullanır.
        
        Args:
            host: Veritabanı sunucu adresi (zorunlu)
            username: Veritabanı kullanıcı adı (zorunlu)
            password: Veritabanı şifresi (zorunlu)
            database_name: Veritabanı adı (varsayılan: "production")
            db_type: Veritabanı tipi (varsayılan: PostgreSQL)
            port: Veritabanı portu (None ise varsayılan port kullanılır)
            high_concurrency: Yüksek eşzamanlılık için optimize et (varsayılan: True)
        
        Returns:
            Production ortamı için DatabaseConfig
        
        Raises:
            InvalidInputError: Gerekli parametreler eksikse
        ...
        Examples:
            ...
            >>> config.engine_config.pool_size  # 50 (high concurrency)
        """
        # Select engine config based on concurrency needs
        if high_concurrency:
            engine_config = EngineConfig.for_high_concurrency()
        else:
            engine_config = EngineConfig()  # Use default (balanced)
