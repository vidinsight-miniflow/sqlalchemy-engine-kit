...
# Module-level logger (DRY: avoid repeated LoggerAdapter.get_logger calls)
...
def _log_function_error(func: Callable, error: Exception, extra: Optional[Dict[str, Any]] = None) -> None:
    """Log function error with consistent format (DRY helper).
    ...
    """
    log_extra = {"function": func.__name__}
    if extra:
...
def _create_decorator_wrapper(
    ...
    def wrapper(*args, **kwargs) -> Any:
        ...
        try:
            ...
            _log_function_error(
                original_func, e,
                extra={"func_module": original_func.__module__ if hasattr(original_func, '__module__') else None}
            )
            raise
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                with mgr.engine.session_context(
                    ...
                    if attempt > 1:
                        _logger.info(f"{original_func.__name__} succeeded on attempt {attempt}/{max_attempts}")
                ...
                from .engine import _is_deadlock_error
                ...
                _logger.warning(
                    f"{original_func.__name__} deadlock on attempt {attempt}, retrying in {wait_time:.2f}s",
                    extra={
                ...
                _log_function_error(original_func, e, extra={"error_type": type(e).__name__})
                raise
        
        _logger.critical(f"Unexpected retry flow for {original_func.__name__}")
...
def _create_inject_wrapper(
    ...
    def wrapper(*args, **kwargs) -> Any:
        ...
        try:
            ...
            _log_function_error(original_func, e)
            raise
