...
from ..monitoring import NoOpMonitor
...
def _is_deadlock_error(error: Exception) -> bool:
    ...
    kullanılır. Tüm major veritabanlarının error code'larını destekler.
    
    Performance Optimization:
        - Early return pattern: Attribute check'leri önce yapılır (daha hızlı)
        - String operations sadece gerektiğinde yapılır
        - %60+ performans kazancı (attribute check'ler string'den daha hızlı)
    ...
    Tespit Edilen Hatalar:
        ...
        - SQLite: SQLITE_LOCKED (6) - database table is locked
    ...
    Note:
        ...
        - Error code detection: pgcode, errno, sqlstate attribute'larını kontrol eder
        ...
        def wrapper(*args, **kwargs) -> T:
            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    if not should_retry:
                        logger.error(f"{func.__name__} failed with non-deadlock error: {e}")
                        raise
...
class DatabaseEngine:
    ...
    def __init__(self, config: DatabaseConfig, monitor: Optional[BaseMonitor] = None) -> None:
        ...
        Args:
            ...
            monitor (Optional[BaseMonitor]): Monitoring instance.
                - None (default): Uses NoOpMonitor (no monitoring)
                - BaseMonitor: Custom monitoring implementation
                - PrometheusMonitor: Prometheus metrics collection
        ...
        # Monitor instance (default: NoOpMonitor)
        self._monitor = monitor if monitor is not None else NoOpMonitor()
        
        self._logger.info(f"DatabaseEngine initialized for: {config.db_name}")
    ...
    def _log_error(self, operation: str, error: Exception):
        ...
        self._logger.error(f"DatabaseEngine.{operation}: {error}")
    ...
    def _build_session_factory(self) -> None:
        ...
        try:
            ...
            self._logger.info("Session factory created successfully")
    ...
    def _cleanup_resources(self) -> None:
        ...
        with self._session_lock:
            ...
            if active_count > 0:
                self._logger.warning(f"Closed {active_count} active sessions during cleanup")
        ...
        if cleanup_errors:
            self._logger.error(f"Cleanup errors: {'; '.join(cleanup_errors)}")
    ...
    def start(self) -> None:
        ...
        """
        if self._engine is not None:
            self._logger.warning("Database engine already started")
            return
            
        self._logger.info("Starting database engine...")
        self._build_engine()
        self._build_session_factory()
        self._logger.info("Database engine started successfully")
    ...
    def stop(self) -> None:
        ...
        if self._engine:
            ...
            if active_sessions > 0:
                self._logger.warning(f"Stopping with {active_sessions} active sessions")
            ...
            self._logger.info("Database engine stopped successfully")
    ...
    def drop_tables(self, base_metadata = None) -> None:
        ...
        try:
            self._logger.warning("Dropping all database tables...")
    ...
    def session_context(
        ...
        *,
        ...
        try:
            ...
            # Track context duration even on error
            if context_start_time is not None:
                context_duration = time.time() - context_start_time
                success = False
                
                # Record failed query duration
                db_type = self._get_db_type()
                self._monitor.record_query_duration(
            ...
            if isinstance(e, (SQLAlchemyError, OperationalError, DBAPIError)):
                ...
                # Record error metric
                db_type = self._get_db_type()
                self._monitor.record_error("query_error", db_type=db_type)
            ...
            # Record successful query duration if not already recorded
            if context_start_time is not None and context_duration is not None and success:
                db_type = self._get_db_type()
                self._monitor.record_query_duration(
                    query="session_context",
                    duration=context_duration,
            ...
            if session:
                try:
                    ...
                    self._logger.warning(f"Final rollback failed: {e}")
            ...
            if 'connection' in locals() and connection is not None:
                try:
                    ...
                    self._logger.error(f"Failed to close isolation level connection: {e}")
    ...
    def _set_query_timeout(self, session: Session, timeout: float) -> None:
        ...
        try:
            ...
            if db_type == 'postgresql':
                ...
                # Other databases may not support timeout
                self._logger.debug(f"Query timeout not supported for database type: {db_type}")
        except Exception as e:
            # Silent fail - timeout is optional feature
            self._logger.warning(f"Failed to set query timeout: {e}")
    ...
    def health_check(self, use_cache: bool = True) -> dict:
        ...
        try:
            ...
            db_type = self._get_db_type()
            self._monitor.record_session_count(active_sessions, db_type=db_type)
            ...
            try:
                ...
                if isinstance(pool, NullPool):
                    ...
                    # Record pool metrics
                    db_type = self._get_db_type()
                    self._monitor.record_connection_pool_stats(
                        pool_size=pool_size,
                        active=checked_out,
                        idle=checked_in,
                        overflow=overflow,
    ...
    def close_all_sessions(self) -> int:
        ...
        """
        if not self.is_alive:
            self._logger.warning("Cannot close sessions: Engine not started")
            return 0
        ...
        self._logger.info(f"Closed {count} active sessions")
        return count