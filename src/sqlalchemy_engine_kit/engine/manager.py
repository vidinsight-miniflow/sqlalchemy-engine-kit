...
class DatabaseManager:
            ...
            with cls._lock:
                ...
                if cls._instance is None and not cls._is_resetting:
                    ...
                    raise DatabaseManagerResetError(
                        message="DatabaseManager is being reset. Please wait and try again."
                    )
    ...
    def initialize(
        ...
        create_tables: Optional[Any] = None,
        ...
        Args:
            ...
            create_tables (Optional[Any]): SQLAlchemy Base.metadata objesi.
                - None (varsayılan): Tablolar oluşturulmaz
                - Base.metadata: Tablolar otomatik oluşturulur (auto_start=True ise)
            ...
            monitor (Optional[BaseMonitor]): Monitoring instance.
                - None (varsayılan): Uses NoOpMonitor (no monitoring)
        ...
        with self._lock:
            if self._initialized:
                if not force_reinitialize:
                    error = DatabaseManagerAlreadyInitializedError()
            ...
            try:
                # Use provided monitor or default to NoOpMonitor
                engine_monitor = monitor if monitor is not None else NoOpMonitor()
                self._engine = DatabaseEngine(config, monitor=engine_monitor)
                self._config = config
                
                if auto_start and create_tables is not None:
                    self._engine.start()
                    self._engine.create_tables(create_tables)
                    self._logger.info("DatabaseManager initialized, tables created and started successfully")
                elif auto_start and create_tables is None:
                    self._engine.start()
                    self._logger.info("DatabaseManager initialized and started successfully")
                elif not auto_start and create_tables is not None:
    ...
    def _ensure_initialized(self) -> None:
        """Ensure manager is initialized or raise DatabaseManagerNotInitializedError (DRY helper).
        
        Raises:
            DatabaseManagerNotInitializedError: If manager is not initialized
        """
    ...
    def engine(self) -> 'DatabaseEngine':
        """Aktif DatabaseEngine instance'ını thread-safe döndürür.
        ...
        with self._lock:
            ...
            # Store reference outside lock to avoid holding lock during use
            engine = self._engine
        return engine
    ...
    def is_started(self) -> bool:
        ...
        with self._lock:
            if not self._initialized or self._engine is None:
                return False
            engine = self._engine
        # Lock dışında is_alive kontrolü (read-only, güvenli)
    ...
    def stop(self) -> None:
        ...
        Idempotent: Initialize edilmemişse veya zaten durdurulmuşsa hata vermez.
    ...
    def get_health_status(self) -> dict:
        ...
        """
        # Thread-safe: Acquire lock and get local reference
        with self._lock:
            engine = self._engine
        ...
        # Use local reference outside lock (safe, engine won't be garbage collected)
        if engine is not None:
            status['engine_started'] = engine.is_alive
            try:
                # Always try to get health check, even if stopped
                status['engine_health'] = engine.health_check()
            except Exception as e:
    ...
    def __repr__(self) -> str:
        ...
        engine_status = ""
        if engine:
            engine_status = f", engine_alive={engine.is_alive}"
    ...
    def __str__(self) -> str:
        ...
        # Use local reference outside lock (safe)
        engine_status = "started" if engine and engine.is_alive else "stopped"
        active_sessions = engine.get_active_session_count() if engine else 0
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if engine is not None:
            try:
                active_sessions = engine.get_active_session_count()
    ...
    def get_instance(
        ...
        if not instance.is_initialized:
            if config is None:
                raise DatabaseManagerNotInitializedError(
                    message="DatabaseManager not initialized and no config provided. "
    ...
    def reload_config(self, new_config: DatabaseConfig, restart: bool = True) -> None:
        ...
        with self._lock:
            if not self._initialized:
                raise DatabaseManagerNotInitializedError(
                    message="Cannot reload config: Manager not initialized"
                )
            
            self._logger.info("Reloading DatabaseManager configuration...")
            
            # Thread-safe: Get local reference to old engine
            old_engine = self._engine
            
            # Önce mevcut engine'i durdur (lock dışında, güvenli)
            if old_engine is not None:
                active_sessions = old_engine.get_active_session_count()
                if active_sessions > 0:
                    self._logger.warning(
            ...
            # Yeni engine oluştur (lock içinde)
            try:
